  ...
  set<pg_shard_t> *to_read)
{
  ...
  dout(0) << "hekang" << dendl;
  //一些可以检测的参数
	int num_coordinated = 0;
	float coordination_rate;
	int unconflict = 0;
	int sched_time = 0, propa_time = 0;//调度时间和分发时间

  //获取redis_context服务器
  redisReply* reply;
  vector<redisContext *> contexts; 
  for(int i = 0; i < NUM_OSD; i++){
    contexts.push_back(osd->redis_contexts[i]);
  }
	...
	vector<int> &Load_List)
{
	...
	if (reply->integer == 1) {//说明里面只有一个head了，可以把comm_queue2删除
        ...
        string reply_string = reply->str;
				...
				res_id = stoi(getstring(reply_string));
				res_time = stoi(getstring(reply_string));
		...
		dout(0) << "find oldest version = " << oldest_time << dendl;
		if (oldest_index.size()) {
			int index = oldest_index[0];//取最早时间戳的第一个
			dout(0) << "res is ok :" << res_strings[index] << dendl;
